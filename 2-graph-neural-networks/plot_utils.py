...
def plot_graph(title, grph_in, coord_id):
    G1 = to_networkx(
        Data(
            x=grph_in[coord_id],
    ...
    node_adjacencies = []
    node_text = []
    for _, adjacencies in enumerate(G1.adjacency()):
...
def plot_bipartite_graph(title, colour, grph_in, edges_to_plot, nodes1, nodes2):
...
def scatter_plot(
    fig, ax, lat: np.array, lon: np.array, data: np.array, cmap: str = "viridis", title = None
) -> None:
    psc = ax.scatter(
        lon,
        lat,
